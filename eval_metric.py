...
imagenet_class_order = open("./imagenet_classes.txt", "r").read().splitlines()
...
def calculate_auroc(df_all, distance, dataset_in, use_clustering=False, percentile=[0.0, 1.0],num_classes=10):
    ...
    if not use_clustering:
        in_class_list = get_class_list('ImageNet_orig' if num_classes==1000 and dataset_in =='ImageNet' else dataset_in)
    else:

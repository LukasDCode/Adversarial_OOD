...
from scipy.spatial.distance import wminkowski,cdist
import pickle
...
def parse_option():
    ...
    parser.add_argument('--dataset', default='cifar10', required=False, help='cifar10 | cifar100 | stl10 | ImageNet30 | ImageNet')
...
def run_model(model, loader,projection, softmax=False,pretrained=False, fc=None):
    ...
    for images, target in loader:
        ...
        if pretrained:
            ...
            emb, classifier = model(images, feat_cls=True)
            cls_list.append(F.softmax(classifier).data.cpu())
        else:
            emb = model(images,return_feat=True if projection == "wdout_projection" else False)

        out_list.append(emb.data.cpu())
...
def get_distances(train_list, test_list, out_list, classes_mean, distance, classes_feats, reg_factor =1, train=True):

    if distance=='Euclidean':
        print('Compute euclidean distance for in and out distribution data')
        if reg_factor==1.0:
            train_dists = euclidean_dist(train_list, classes_mean) if train else None
            ...
            train_dists = norm_euclidean_dist(train_list, classes_mean, )  if train else None
    ...
    if distance =="Mahalanobis":
        ...
        train_dists = mahalanobis(train_list, classes_mean, sup_inv_cov) if train else None
    ...
    if distance== "Cosine":
        ...
        train_dists = cosine(train_list, classes_mean, )if train else None
...
def main(opt, cache=None, eval_over_time_pkl=None):
    ...
    # load networks
    fc=None
    pretrained =  'pretrained' in opt.ckpt
    if opt.model=='vit':
        ...
        if pretrained:
            ...
            fc = torch.nn.Linear(num_ftrs, opt.num_classes)
        else:
        ...
        # load checkpoint
    if opt.num_classes==1000 and opt.dataset=='ImageNet':
            state_dict = load_checkpoint(opt.ckpt, new_img=opt.image_size, emb_dim=opt.emb_dim,
                                         layers=opt.num_layers, patch=opt.patch_size)
            print("Loading pretrained weights from {}".format(opt.ckpt))
        ...
        if pretrained:
            ...
            fc.weight = torch.nn.Parameter(ckpt['model']['fc.weight'])
            fc.bias = torch.nn.Parameter(ckpt['model']['fc.bias'])
        else:
            model.load_state_dict(ckpt['state_dict'] if opt.model=="vit" else ckpt['model'], strict=False)
    model = model.cuda()
    if fc is not None:
    ...
    if cache is  None:
        ...
        if opt.use_train:
            train_emb, train_targets, train_sfmx = run_model(model,train_loader, opt.projection,
                                                             'supce' in opt.ckpt.lower(),pretrained= pretrained, fc=fc)
    ...
    # open_file = open('./im_cls_mn', "rb")
    # classes_mean = pickle.load(open_file)
    # open_file.close()
    ...
    if opt.distance=="Softmax":
        train_dist=train_sfmx if opt.use_train else None; test_dist=test_sfmx; out_dists=out_sfmx;
    else:
        train_dist, test_dist, out_dists, dist_mean = get_distances(train_emb, test_emb, out_list, classes_mean, opt.distance, classes_feats,
                                                   opt.reg_factor, train = opt.use_train)

    in_dtypes = ['test','train'] if opt.use_train else ['test']
    ...
    if opt.use_train:
        ...
        auroc, tnr_at_tpr95, aupr = calculate_auroc(dfs, opt.distance, opt.dataset, num_classes=opt.num_classes)
    ...
    with open(os.path.join(os.path.dirname(opt.ckpt), "auroc.txt"), "a") as output:
        ...
        output.close()
    return cache,  eval_over_time_pkl if opt.use_train else None
...
def model_arch(opt, ckpt_path):
    ...
    if 'resnet' in ckpt_path and '224' not in ckpt_path:
...
def run_ood_distance(opt):
    ...
    im_out_data_list = ['SVHN','Places365','Dtd','Dogs','Food','Caltech256', 'CUB',]
    ...
    for ckpt_path in ckpt_path_list: # set all dataset param for diff train model
        ...
        elif 'cifar10' in ckpt_path:
            opt.dataset = 'cifar10';opt.num_classes = 10;out_dataset_list = cifar10_out_data_list
        else :
            opt.dataset='ImageNet';opt.num_classes =1000;opt.data_dir='data/LSVRC2015';out_dataset_list=im_out_data_list
